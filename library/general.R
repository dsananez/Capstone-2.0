...
wd <- "C:/Users/Daniel/Google Drive/Coursera/Data Science Spec/Capstone 2"
setwd(wd)
...
#clean corpus function (remove numbers, punctuation, white space and transform everything to lower case)
cleanCorpus <- function(x){
  corpusClean <- tm_map(x, removeNumbers)
  temp <- tm_map(corpusClean, removePunctuation)
  temp <- tm_map(corpusClean, stripWhitespace)
  temp <- tm_map(corpusClean, content_transformer(tolower))
  return(temp)
}
...
fourFunct <- function(input){
  input <- cleanText(input)
  input <- strsplit(input, " ")
  if (length(input[[1]]) > 2){
    input <- input[[1]]
    input <- paste(c("^", paste(input[(length(input)-2):length(input)], collapse=" "), " "), collapse="")
    temp <- -sort(-table(fourgrams[grepl(input, fourgrams)]))
    output <- NULL
    for (i in 1:3){
      output[i] <- strsplit(names(temp[i]), " ")[[1]][4]
    }
    return(output)
  } else{
    return(NA)
...
triFunct <- function(input){
  input <- cleanText(input)
  input <- strsplit(input, " ")
  if (length(input[[1]]) > 1){
    input <- input[[1]]
    input <- paste(c("^", paste(input[(length(input)-1):length(input)], collapse=" "), " "), collapse="")
    temp <- -sort(-table(trigrams[grepl(input, trigrams)]))
    output <- NULL
    for (i in 1:3){
      output[i] <- strsplit(names(temp[i]), " ")[[1]][3]
    }
    return(output)
  } else{
    return(NA)
...
biFunct <-function(input){
  input <- cleanText(input)
  input <- strsplit(input, " ")
  if (length(input[[1]]) > 0){
    input <- input[[1]]
    input <- paste(c("^", paste(input[length(input)], collapse=" "), " "), collapse="")
    temp <- -sort(-table(bigrams[grepl(input, bigrams)]))
    output <- NULL
    for (i in 1:3){
      output[i] <- strsplit(names(temp[i]), " ")[[1]][2]
    }
    return(output)
  } else{
    return(NA)
...
uniFunct <-function(input){
  return(c("the", "to", "and", "a"))
}

#Predictie Model
predictiveModel <- function(input, prof = FALSE){
  temp <- fourFunct(input)
  temp <- c(temp, triFunct(input))
  temp <- c(temp, biFunct(input))
  temp <- c(temp, uniFunct(input))
  temp <- (unique(temp[!is.na(temp)]))
  temp <- 
    return(head(temp[!grepl("^<", temp)],4))
}
